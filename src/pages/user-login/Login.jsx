...
const Login = () => {
    ...
    const [loading, setLoading] = useState(false); // only call api
    ...
    const onOtpSubmit = async () => {
        try {
            ...
            const otpString = otp.join(''); // convert otp in string
            let response;
            ...
            console.log(error);
            setError(error.message || "Failed to verified OTP");
        } finally {
            setLoading(false);
        }
    }
    ...
    const onProfileSubmit = async (data) => {
        try {
            setLoading(true);
            const formData = new FormData(); // in build funnction 
                ...
                {step === 1 && (
                    <form className='space-y-4' onSubmit={handleLoginSubmit(onLoginSubmit)}>
                        ...
                        <div className='relative'>
                            <div className='flex'>
                                <div className='realtive w-1/3'>
                                    <button
                                        type="button" className={`flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center ${theme === "dark" ? "text-white bg-gray-700 border-gray-600" : "text-gray-900 bg-gray-100 border-gray-300"} border rounded-s-lg hover:bg-gray-200 focus:ring-4 focus-outline-none focus:ring-gray-100`}
                                    ...
                                    {showDropdown && (
                                        <div className={`absolute z-10 w-full mt-1 ${theme === "dark" ? "bg-gray-700 border-gray-700" : "bg-white border-gray-300"} border rounded-md shadow-lg max-h-60 overflow-auto`}>
                                            <div className={`sticky top-0 ${theme === "dark" ? "bg-gray-700" : "bg-white"} p-2`}>
                                                <input
                                                    ...
                                                    className={`w-full px-2 py-1 border ${theme === "dark" ? "bg-gray-600 border-gray-700 text-white" : "bg-white border-gray-300"} rounded-md text-sm focus:outline-none focus:rihgt-2 focus:ring-green-500`}
                                                />
                                            </div>
                                ...
                                <input
                                    ...
                                    className={`w-2/3 px-4 py-2 border ${theme === 'dark' ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300"} rounded-md focus:outline-none focus:rihgt-2 focus:ring-green-500 ${loginErrors.phoneNumber ? "border-red-500" : ""}`}
                                />
                        ...
                        <div className={`flex items-center border rounded-md px-3 py-2 ${theme === "dark" ? "bg-gray-700 border-gray-600" : "bg-white border-gray-300"}`}>
                            ...
                            <input
                                ...
                                className={`w-full bg-transparent focus:outline-none ${theme === 'dark' ? " text-white" : "text-orange-800"} ${loginErrors.email ? "border-red-500" : ""}`}
                            />
                            {loginErrors.email && (
                    ...
                    <form onSubmit={handleProfileSubmit(onProfileSubmit)} className='space-y-4'>
                        <div className='flex flex-col items-center mb-4'>
                            <div className='relative w-24 h-24 mb-2'>
                                ...
                                />
                                <label htmlFor='profile-picture' className='absolute bottom-0 right-0 bg-green-500 text-white p-2 rounded-full cursor-pointer hover:bg-green-500 transition duration-300'>
